...
/*
	BarScan()
	Moves mouse left and right to detect the ends of
	a colored bar and returns four byref variables.
	Used for scanning HP, Mana, Target, Pet, etc

	BarScan(Color, BarY, LeftX, RightX)
*/
BarScan(ByRef Color, ByRef BarY, ByRef LeftX, ByRef RightX) {
    ...
    Loop
    {
        xpos--
        MouseMove, %xpos%, %ypos%
        PixelGetColor, Color2, %xpos%, %ypos%
        If Color != %Color2%
        Break
    }
    LeftX := xpos+1
...
^Home:: ; Ctrl+SPACE starts Health color scan
...
FindText(ByRef x:="FindTextClass", ByRef y:="", args*)
{
  ...
  return (x=="FindTextClass") ? obj : obj.FindText(x, y, args*)
}
  ...
  , zoomW:=1, zoomH:=1)
{
  ...
  , VarSetCapacity(s1, k*4), VarSetCapacity(s0, k*4), VarSetCapacity(ss, w*(h+2))
...
{
  local
  VarSetCapacity(allpos, ini.allpos_max*8), zoomW:=ini.zoomW, zoomH:=ini.zoomH
...
PicInfo(text)
{
  ...
  if (mode=5)
  {
    ...
    if (v~="[^\s\w\-/,]") && FileExist(v)  ; ImageSearch
    {
      ...
      arr:=StrSplit(r,"-"), n:=arr.HasKey(1)?Floor(arr.RemoveAt(1)):0
...
ObjView(obj, keyname:="")
{
  ...
  Gui, +LastFound +AlwaysOnTop +Hwndid
...
Gui(cmd, arg1:="", args*)
{
  ...
  Case "Capture", "CaptureS":
    ...
    if (Event="OK")
    {
      if (!A_IsCompiled)
        ...
        s:=this.GetScript()
      if RegExMatch(s,"Oi)FindText[^\n]+args\*[\s\S]*?Script_End[(){\s]+}",r)
        s:="`n;==========`n`n`n" r[0] "`n"
  ...
  Case "Test", "TestClip":
    ...
    else
    {
      ...
      r:=StrSplit(Lang["s8"] "||||", "|")
      MsgBox, 4096, Tip, % r[1] ":`t" (!v?0:v.Length()) "`n`n"
