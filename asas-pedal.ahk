...
Col3W := 150
Col4W := 150
LVW := Col1W + Col2W + Col3W + Col4W + 5
Gui, LV:Default
Gui, Margin, 10, 
Gui, Add, Text, CBlue, Ctrl+SolClick ile satir ^ sutun ' lardan skill secimi yapiniz.
Gui, Add, ListView, w%LVW% r10 -Multi +Grid +LV0x010000 hwndHLV vVLV gListViewEvents HwndHLV, [SkillBar]|[F1]|[F2]|[F3]
Gui Add, CheckBox, hWndhChkCheckbox vChkCheckbox x550 y20 w120 h23, F1-F3 atak.
Gui Add, CheckBox, hWndhChkCheckbox2 vChkCheckbox2 x550 y40 w120 h23, Minor ^ MPpot.
...
Loop ; skilerin satir sutun bulup kaydetmesi lazim tus gonderebilmesi icin. sadece listview icerisinde secilince o anlik kayit ediyor. simdilik ara veriyorum, piyasadaki pedallardan daha iyi, skill okudugu icin. skill aktif oldugu anda basiyoruz ahk ile.
     ; spike dan okumaya baslayip ezik skilere dogru gidiyoruz, bu sure zarfinda eger yuksek skilerden birisi aktif olursa direkt ona geciyor, sonra kalanlara devam ediyor. amac max damage alabilmek.
{
...
ExitApp
; ======================================================================================================================
DDLEvents: ; ListView icerisini guncelleme.
GuiControlGet, asas
If (asas <> "") {
   Gui, LV:Default
   LV_Modify(CBB_Row, "Col" . CBB_Col, asas)
...
WM_LBUTTON(W, L, M, H) {
   ...
   Gui, DDL:Hide
   If (H = HLV) && (W & 0x0008) { ; Ctrl+LBUTTON ListView icerisinde secim yapma.
      IF (SI := LV_EX_SubItemHitTest(H, L & 0xFFFF, L >> 16)) && (SI = 2) {
         ...
         If (asas <> "")
            GuiControl, DDL:ChooseString, asas, %asas%
         Else
            GuiControl, DDL:Choose, asas, 0
   ...
   If (H = HLV) && (W & 0x0008) { 
      IF (SI := LV_EX_SubItemHitTest(H, L & 0xFFFF, L >> 16)) && (SI = 3) {
         CBB_Row := ErrorLevel + 1
         CBB_Col := SI
         LV_Modify(CBB_Row, "Focus Select")
         LV_GetText(asas, CBB_Row, CBB_Col)
   ...
   If (H = HLV) && (W & 0x0008) { 
      IF (SI := LV_EX_SubItemHitTest(H, L & 0xFFFF, L >> 16)) && (SI = 4) {
         CBB_Row := ErrorLevel + 1
         CBB_Col := SI
         LV_Modify(CBB_Row, "Focus Select")
         LV_GetText(asas, CBB_Row, CBB_Col)
