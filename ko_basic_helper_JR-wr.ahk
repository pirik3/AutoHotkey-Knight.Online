...
Loop
{
	if (ok:=FindText(X, Y, 1229, 441, 1589, 648, 0, 0, kalkan_selfname))
	{
		MouseGetPos, x1, y1
		MouseMove, X, Y
		Send, {RButton down}{RButton up}{RButton down}{RButton up}
		Sleep, 50
		MouseMove, x1, y1
	}
	else
...
Loop
{
	if (ok:=FindText(X, Y, 1229, 441, 1589, 648, 0, 0, dks))
	{
		MouseGetPos, x1, y1
		MouseMove, X, Y
		Send, {RButton down}{RButton up}{RButton down}{RButton up}
...
Loop
{
  PixelSearch, Px, Py, 948, 31, 1046, 70, FFFF00, 5, Fast 
  if (ErrorLevel)
    {
      ToolTip, JR yazisi bekleniyor.
      Send, {LButton down}
    }
    else
    {
